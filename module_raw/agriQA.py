...
from .CallingBREEZE import QuestionExpansionHYDEWithBREEZE, SummaryGeneratorWithBREEZE, QuestionTypeClassificationWithBREEZE
...
def agriQA_with_TAIDE(retrieval_hint, question, confidence = 0.8):
    """sepefisic function for agriQA with TAIDE
    """
...
def agriQA_with_BREEZE(retrieval_hint, question, confidence = 0.8):
    """sepefisic function for agriQA with BREEZE
    """
    question_classification_result = QuestionTypeClassificationWithBREEZE(question)

    if question_classification_result == "Non-Agriculural Question":
        content={}
        result =["很抱歉，這不是個農業問題，神農BREEZE身為一個針對農業知識問答強化的語言模型，無法處理這樣的問題。"]
        return result, [content]
        ...
        # print("BREEZE Model")
        # # hyDE_query = QuestionExpansionHYDEWithBREEZE(question) #BREEZE 
        # print(hyDE_query)
        # relevant_contexts = sparse_retriever(retrieval_hint+" "+hyDE_query,retrieve_K)   #使用sparse retrievier
        # print("Enable BREEZE HYDE Query Expansion")
        relevant_contexts = sparse_retriever(retrieval_hint,retrieve_K)   #使用sparse retrievier
        ...
        # find answer-context pair
        answer_context_pair = {}
        for answer, context in zip(answers_by_ExtractiveLM, contexts_by_ExtractiveLM):
            answer_context_pair[answer] = context 
        answer_context_pair = [answer_context_pair]
        ...
        return  answers_by_ExtractiveLM, answer_context_pair
